    ...
    void BuildFastSearchList() {
        ...
        for (int i = 0; i < list_size; ++i) {
            ...
            for (int k = 0; k < fast_search_list_size; ++k) {
                cur->fast_search_list[k] = step;
                // step points to 2^k steps after cur
                // To get 2^(k+1) steps, we go 2^k steps from step
                // But wait, it's easier to pre-calculate if we have them all
            }
            cur = cur->next;
        }

        // Proper jump table construction
        for (int k = 0; k < fast_search_list_size; ++k) {
            cur = head;
            for (int i = 0; i < list_size; ++i) {
                if (k == 0) {
                    cur->fast_search_list[k] = cur->next;
                } else {
                    cur->fast_search_list[k] = cur->fast_search_list[k-1]->fast_search_list[k-1];
                }
                cur = cur->next;
            }
        }
...
public:
    ...
    explicit SpeedCircularLinkedList(std::vector<int> node_bounds) {
        ...
        for (int bound : node_bounds) {
            Node* newNode = new Node(bound, fs_size);
            if (head == nullptr) {
                head = newNode;
            } else {
                prev->next = newNode;
...
private:
    Node* findNode(int code) {
        ...
        int fs_size = log2(list_size);
        Node* cur = head;
        for (int k = fs_size - 1; k >= 0; --k) {
            if (cur->fast_search_list[k]->bound < code) {
                cur = cur->fast_search_list[k];
            }
        }
        // After jumping, the target is either the current node's next or further
        // but since we jumped as far as possible without reaching or passing code,
        // it must be the next node.
        return cur->next;
    }
};
